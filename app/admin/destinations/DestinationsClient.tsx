...
type ModalState =
  ...
  | { mode: "delete"; destination: Destination }
  | null
...
export default function DestinationsClient({
  ...
  const openEdit = (dest: Destination) => {
    setForm({
      ...
      rating: dest.rating != null ? String(dest.rating) : "",
    })
    setError(null)
    setModal({ mode: "edit", destination: dest })
  }
    ...
    <>
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-white/40">{destinations.length} total</p>
        </div>
        <motion.button
            ...
            <tbody>
              {destinations.map((dest, i) => (
                ...
                >
                  ...
                  <td className="px-5 py-4 text-sm text-white/55">{dest.location}</td>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => { setError(null); setModal({ mode: "delete", destination: dest }) }}
              ...
              <div className="px-8 py-7">
                ...
                <form onSubmit={handleFormSubmit} className="flex flex-col gap-4">
                  <div className="grid grid-cols-2 gap-4">
                    <Field label="Name" value={form.name} onChange={setField("name")} placeholder="Santorini Escape" required />
                    <Field label="Location" value={form.location} onChange={setField("location")} placeholder="Santorini, Greece" required />
                  </div>
                  <Field label="Description" value={form.description} onChange={setField("description")} placeholder="A short description…" />
                  <Field label="Image URL" value={form.image_url} onChange={setField("image_url")} placeholder="https://…" />
                  <div className="grid grid-cols-2 gap-4">
                    <Field label="Price (USD)" value={form.price} onChange={setField("price")} type="number" placeholder="2499" required />
                  ...
                  <div className="mt-2 flex gap-3">
                    ...
                    >
                      {isPending ? "Saving…" : modal?.mode === "add" ? "Add Destination" : "Save Changes"}
                    </motion.button>
                  </div>
                </form>
              </div>
      ...
      <AnimatePresence>
        {modal?.mode === "delete" && (
          <>
            ...
            >
              <div className="mb-2 text-base font-bold text-white">Delete destination?</div>
              <p className="mb-6 text-sm text-white/45">
                <span className="text-white/70">&ldquo;{modal.destination.name}&rdquo;</span> will be permanently removed.
              </p>
